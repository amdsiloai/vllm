...
class QuantFP8(CustomOp):
    ...
    def _quantize_group_native_rmsnorm(
        ...
        assert not self.column_major_scales, (
            "column major for scales is not supported in this kernel"
        )
        ...
        ## following asserts assume x and z were created with torch.zeros()
        ...
        assert self.group_size == head_dim, (
            "the kernel is only supported for group_size == head_dim, "
            f"got group_size {self.group_size} and head_dim {head_dim}"
        )
