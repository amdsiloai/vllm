...
class GatedDeltaNetAttention(PluggableLayer, MambaBase):
    ...
    def rearrange_mixed_qkv(self, mixed_qkv):
        ...
        # 4. Zero cost reshape
        query = q_contig.view(1, n_seq, -1, self.head_k_dim)
        key = k_contig.view(1, n_seq, -1, self.head_k_dim)
        value = v_contig.view(1, n_seq, -1, self.head_v_dim)
    ...
    def forward_cuda(
        ...
        # we use torch.empty, then we fill the tensor with 0 for the padding
        # positions in the gdn kernel, so that we can remove the autogenerated
        # kernel caused by torch.zeros

        ## create buffer for core_attn_out and z, both with shape
        ## (num_tokens, num_v_heads // tp_size, head_v_dim)
    ...
    def _forward_core(
        ...
        if (
            ...
            mixed_qkv, z_prep, b_prep, a_prep = self.prepare_gdn_attention_core_inputs(
        ...
        if attn_metadata.num_prefills > 0:
            ## TODO: fuse rearrange with chunk kernel as well
            query_non_spec, key_non_spec, value_non_spec = self.rearrange_mixed_qkv(
                mixed_qkv_non_spec
            )
