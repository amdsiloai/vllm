...
def fused_reshape_causal_conv1d_update_fast(
    ...
    assert z_out.is_contiguous(), (
        "z_out should be contiguous if it was created by torch.zeros()"
    )
    assert core_attn_out.is_contiguous(), (
        "core_attn_out should be contiguous if it was created by torch.zeros()"
    )
    x = x.view(x.shape[0], -1)
    ba = ba.view(ba.shape[0], -1)
    assert z_out.size() == core_attn_out.size(), (
        "z_out and core_attn_out should have the same shape"
    )
